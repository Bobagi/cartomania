#!/usr/bin/env bash
# Deploy helper for Cartomania (https://cartomania.bobagi.space).
...
deploy_front() {
  ...
  ok "frontend live (pm2 cartomania-web :3055)"
}

deploy_back() {
  step "Building + recreating backend container"
  docker compose build cartomania || die "backend build failed"
  docker compose up -d cartomania || die "backend up failed"
